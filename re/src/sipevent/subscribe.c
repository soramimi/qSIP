...
static int sipsub_alloc(struct sipsub **subp, struct sipevent_sock *sock,
			...
			const char *event, const char *accept, const char *id, uint32_t expires,
			const char *cuser,
...
int sipevent_subscribe(struct sipsub **subp, struct sipevent_sock *sock,
			   ...
			   const char *from_uri, const char *event, const char *accept, const char *id,
...
int sipevent_dsubscribe(struct sipsub **subp, struct sipevent_sock *sock,
			struct sip_dialog *dlg, const char *event, const char* accept,
		   ...
		   const char *fmt, ...)
{
	...
	err = sipsub_alloc(subp, sock, true, NULL, uri, from_name, from_uri,
			   "refer", NULL, NULL, DEFAULT_EXPIRES, cuser,
			   routev, routec, authh, aarg, aref, forkh, notifyh,
			   closeh, arg, fmt, ap);
	va_end(ap);
		  ...
		  void *arg)
{
	...
	sub->accept  = mem_ref(osub->accept);

...
int sdp_media_alloc(struct sdp_media **mp, struct list *list, const struct sa *remote_addr)
{
	int err;
	struct sdp_media *media;
	err = media_alloc(&media, list);
	if (err == 0) {
		media->raddr = *remote_addr;
		media->ldir = media->rdir = SDP_SENDRECV;
		*mp = media;
	}
	return err;
}

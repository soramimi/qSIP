 ...
 */
static void fd_handler(struct re *re, int fd, int flags, void *user_data)
{
	...
	re->fhs[fd].fh(flags, re->fhs[fd].arg, user_data);
 ...
 * @return 0 if success, otherwise errorcode
 */
static int fd_poll(struct re *re, void *user_data)
{
	...
	for (i=0; (n > 0) && (i < re->nfds); i++) {
		...
		if (re->fhs[fd].fh) {
#if MAIN_DEBUG
			fd_handler(re, fd, flags, user_data);
#else
